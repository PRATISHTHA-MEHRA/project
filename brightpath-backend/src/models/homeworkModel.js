  ...
  async addHomework(hw) {
    const result = await pool.query(
      `INSERT INTO homework_assignments
        (batch_name, subject, teacher_name, title, description, due_date, submitted_count, pending_count, status, attachment_name)
      ...
      [
        ...
        hw.submitted || 0, hw.pending || 0, hw.status || "Active", hw.attachment || null
      ]
    );
  ...
  async editHomework(code, hw) {
    const result = await pool.query(
      `UPDATE homework_assignments SET
        ...
        description = $5, due_date = $6, status = $7, updated_at = NOW()
       WHERE homework_code = $8
       RETURNING
        homework_code   AS id,
        batch_name      AS batch,
        subject,
        ...
        status,
        attachment_name AS attachment`,
      [hw.batch, hw.subject, hw.teacher, hw.title, hw.desc, hw.due, hw.status || "Active", code]
    );
    return result.rows[0];
  },
